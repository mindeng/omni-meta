...
IPTC codec · ICC 摘要 · TIFF 顶层格式 · **流式 `EventParser`（push 式 raw 条目流 + 提前终止，里程碑 G）** · **XMP sidecar 合并（`Metadata::with_xmp_sidecar`，里程碑 H）** · async/tokio 适配器 · `video_codec`/`audio_codec` 等 Unified 字段扩展
...
### 里程碑 H — XMP sidecar 合并（解析后注入 `.xmp` 旁挂文件）⬜ 设计已定 — 设计 `specs/2026-06-28-xmp-sidecar-merge-design.md`
...
- [ ] `Metadata::with_xmp_sidecar(self, &[u8], Limits) -> Self`：解析后 method（否决 `Options` 注入——避免时间耦合 + 流式别扭），sidecar 字节仅借用单次调用
- [ ] `RawTags.xmp_sidecar: Vec<XmpProperty>` 单列（留 provenance + 独立优先级档）；`Metadata.structural: StructuralFields` 快照（`pub(crate)`，`StructuralFields` 仍内部类型）使 merge 重规整与「一次到位」字节级一致
- [ ] normalize 插档：描述字段（title/description/creator/copyright）**sidecar>内嵌**；技术字段（make/model/dims/created/gps）**内嵌>sidecar**（sidecar 仅 XMP 兜底）
- [ ] keywords 留 raw 层（Unified 无字段 + 受 ≥2 来源铁律阻塞）
- [ ] 测试：两路径一致性、描述胜/技术兜底、provenance 隔离、退化告警、幂等叠加、no_std
- [ ] **不在本期**：fs/路径发现、IPTC-IIM、Unified keywords、独立 `FileFormat::Xmp`、写回
