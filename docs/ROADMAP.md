...
### 里程碑 H — XMP sidecar 合并（解析后注入 `.xmp` 旁挂文件）✅ 完成 — 设计 `specs/2026-06-28-xmp-sidecar-merge-design.md` / 计划 `plans/2026-06-28-xmp-sidecar-merge.md`

**为什么做**：服务 Apple Photos `Export IPTC as XMP` 场景——用户写的 title/caption/creator/copyright/keywords 落在旁挂 `.xmp`（现代 IPTC=XMP 形态，非 IIM）。raw-only 小增量，不阻塞他者；与里程碑 B（IPTC-IIM）正交。库不碰文件系统，sidecar 发现归调用方，API 只收 `&[u8]`。

- [x] `Metadata::with_xmp_sidecar(self, &[u8], Limits) -> Self`：解析后 method（否决 `Options` 注入——避免时间耦合 + 流式别扭），sidecar 字节仅借用单次调用；告警去重靠 `truncate` 回退（normalize 只追加 + sidecar 回退静默）
- [x] `RawTags.xmp_sidecar: Vec<XmpProperty>` 单列（留 provenance + 独立优先级档）；`Metadata.structural: StructuralFields` 快照（`pub(crate)`；`Metadata`/`FileFormat` 派生 `Default` 以解外部 crate 字面量构造）使 merge 重规整与「一次到位」字节级一致
- [x] normalize 插档：描述字段（title/description/creator/copyright）**sidecar>内嵌**；技术字段（make/model/dims/gps）**内嵌>sidecar**（sidecar 仅 XMP 兜底，结构源仍末尾覆盖）
- [x] keywords 留 raw 层（Unified 无字段 + 受 ≥2 来源铁律阻塞）
- [x] 测试：两路径一致性、描述胜/技术兜底（含 orientation/dims/gps）、provenance 隔离、退化告警、幂等叠加、no_std bare-metal
- [ ] **不在本期**：`created` 的 sidecar 兜底（需 XMP ISO-8601 datetime 解析器）、fs/路径发现、IPTC-IIM、Unified keywords、独立 `FileFormat::Xmp`、写回
