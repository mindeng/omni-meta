...
- [ ] **待评估：PNG `tEXt`/`zTXt` 注册关键字** — 现 PNG 解析器只认 `IHDR`/`eXIf`/`iTXt`（`formats/png.rs:76`），`tEXt`/`zTXt` 整块跳过。其**注册关键字**（`Author`→creator / `Software`→software / `Creation Time`→created / `Copyright` / `Description` / `Comment` / `Title`）有明确语义、可投影，现实 PNG 常用 → **读取缺口**；且对里程碑 F **隐私剥离是盲区**（tEXt/zTXt 可携 PII，strip 当前不碰）。注意 `zTXt` 为 zlib 压缩，本库零依赖不解压（与 `iTXt` 压缩标记同策略：警告跳过）。Unified 纳入仍受 ≥2 来源约束（raw 层可先收）。评估范围含**读 + strip 两侧**。
