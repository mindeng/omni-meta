...
- 期望值写成 Rust 常量，与样本字节并置于 `omni-meta-fixtures`：
  ...
  pub struct GoldenSample {
      ...
      /// exiftool 也报告的关键 raw 标签子集（定向锚定 codec 层，非全量）。
      /// 形如 (容器/codec 定位, 期望值)；断言「包含且相等」，容忍额外标签。
      pub raw_subset: Vec<GoldenRawTag>,
  }
  pub fn golden_corpus() -> Vec<GoldenSample> { /* ... */ }
  ```

  `GoldenRawTag` 的具体形态在实现期定（需匹配 `RawTags` 的实际结构——EXIF 条目 /
  XMP 属性 / 容器标签各有定位方式）；原则是用「能在 `RawTags` 里定位一个标签 + 其期望值」
  的最小表示，断言为子集相等。
...
2. **Unified 真相锚定**：`read_slice(bytes).unified` 的**每个期望字段**严格等于 `sample.unified`
   对应字段，且 `format == sample.format`。
3. **raw 子集锚定（codec 层外部真相）**：`raw_subset` 中每个期望标签必须在 `read_slice(bytes).raw`
   中**存在且值相等**（子集相等，容忍额外标签）。这是**唯一用外部真相（exiftool）锚定 codec 层
   的断言**——直击同源偏差最致命处（见 §3.4 缘由）。

### 3.4 断言粒度与口径

- **Unified**：断言 `unified` 字段 + `format`——受控增长的 Unified 契约。
- **raw**：做 exiftool 交集的**子集相等**（§3.3 第 3 步），**不**做全量相等。
  - 缘由：`assert_all_equal` 的 raw 一致只是**适配器 vs 适配器**（四者共用同一 codec），
    锚不住对**外部真相**的正确性；而 raw/codec 层恰是**同源偏差最致命**之处——合成 fixture
    用解析器同样的字节理解构造标签，构造错、解析也错、四适配器照样一致。黄金样本要打破的正是这个，
    故必须有 exiftool 对 raw 的外部锚定。
  - 但**不做全量相等**：真实文件 raw 标签体积大（MakerNotes/缩略图 IFD 等几十项）、随生成工具
    版本漂移、易脆。子集相等兼得「外部锚定 codec 层」与「抗漂移、低维护」——conformance 套件的标准做法。
- **warnings**：**不**断言，best-effort 诊断，与既有 oracle 口径一致。
...
- 不碰任何解析器 / 格式 / codec **逻辑**——纯测试与 CI 基座。
  - **范围修订（2026-06-17 执行期）**：加 fmt/clippy 门禁前，须先清理仓库**预存**的 fmt/clippy 漂移
    （`strip_blocking.rs` import 排序、`bmff.rs` 等处 collapsible-if/Default-field-assignment 共 8 lint）。
    这些是**纯格式/lint 修复、零行为变化**，作为前置提交单列；虽触及解析器文件，但不改任何逻辑。
- 不引入快照库（如 `insta`）：期望值用手写 Rust 常量，零新依赖，契合 core 零依赖的取向。
- 不做 async/tokio 第五适配器（里程碑 D，另立）。
- 黄金样本不追求穷举每字段，只锚定每家族的代表性 Unified 字段 + raw 关键标签子集；后续可增量补。
...
- [ ] `golden_corpus()` 暴露样本；`omni-meta/tests/golden.rs` 对每个样本跑四适配器一致 + Unified/format 锚定 + raw 子集锚定（exiftool 交集），全绿。
- [ ] 每个样本的 `raw_subset` 至少含 1 个 exiftool 核对过的关键标签（图片样本含 EXIF/XMP 标签；GPS 样本含 GPS 原值）。
