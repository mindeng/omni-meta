...
> **本任务暴露并修复的 `driver.rs` 缺陷**（相邻零间隔抽取目标）：EXIF 与 XMP 在 mdat 中紧邻，
> `pull_extract` 消费完 EXIF（consumed>0）后 `SeekTo(xmp_off)` 恰为当前绝对位置 → `skip_remaining==0`。
> 原 `StreamDriver` SeekTo 守卫 `if self.skip_remaining == 0` 会误判为零前进卡死、发 `Truncated` 并停，
> 导致 blocking/seek/push 漏掉 XMP，而 slice（`drive_slice`，SeekTo 忽略 consumed）正确 → 四适配器分歧被差分测试逮到。
> **修复**：守卫改为 `if self.skip_remaining == 0 && consumed == 0`（仅 consumed==0 才是真卡死；consumed>0 是合法的相邻目标，放行 `continue` 重拉；per-drive budget 仍兜底防真死循环）。
> 并在 `driver.rs` 加聚焦回归单测 `stream_consecutive_zero_gap_seek_with_consumed_progresses`（消费 4 字节后零跳 SeekTo 须继续推进）。
